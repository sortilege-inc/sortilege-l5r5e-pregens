...
def main(camp):
    ...
    for d in docs:
        ...
        if legacy and not a.get("accoutrement_name"):
            a["accoutrement_name"] = next((g["name"] for g in t.get("gear") or [] if g.get("accoutrement")), None)
        if not a.get("accoutrement_name") and a.get("first_impression"):
